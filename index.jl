...
begin
	...
	import Enzyme, FastDifferentiation, Mooncake, ReverseDiff, Tracker, Zygote
	using NNlib: softplus, softmax
end

# ╔═╡ bf35dd1f-0dc9-4261-9acc-e104088c3d6e
function run_graph(initial_variables::AbstractVector, n_operations::Integer=10^5)
	nodes = copy(initial_variables)
	for op in 0:n_operations-1
		case = op % 4
		if case == 0
			push!(nodes, softplus(nodes[end-10]))
		elseif case == 1
			push!(nodes, sum(@view(nodes[end-30:5:end-10])))
		elseif case == 2
			push!(nodes, nodes[end-20] * nodes[end-10])
		else
			softmaxes = softmax(@view(nodes[end-4:end]))
...
# ╔═╡ c90d2ebb-c24d-4988-90ea-f14f51339957
init_vars = collect(1.0:100.0)

# ╔═╡ dd84cb79-21b4-4cf4-a4cb-c79079f0486a
scen = Scenario{:gradient, :out}(run_graph, init_vars)
...
data = benchmark_differentiation(backends, [scen]; benchmark=:full)
...
manifest_format = "2.0"
project_hash = "0daa2014586a5d203f9f6b3a4bb722d497c3175f"
...
git-tree-sha1 = "7aead3b922317be569cb24ed2f3d99363f4d0e02"
...
git-tree-sha1 = "09000e0bdec3aed6e3dff553c9fb010d27de56ee"

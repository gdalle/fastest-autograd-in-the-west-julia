...
# ╔═╡ bf8995fa-a9de-11ef-16a7-3b4c53f5cc4f
begin
	using Chairmarks
	using CairoMakie
	using DifferentiationInterface, DifferentiationInterfaceTest
	using DataFrames, DataFramesMeta
	import Enzyme, FastDifferentiation, FiniteDiff, ForwardDiff, Mooncake, PolyesterForwardDiff, ReverseDiff, Symbolics
	using Measurements
	using NNlib: softplus, softmax
	using PlutoUI
	using Statistics
	using Test
	using Base.Threads
...
# ╔═╡ 9c463c8c-0a01-44bb-bdf9-7586e88673c1
md"""
...
# ╔═╡ c4a3c2bf-2a16-4bfc-8c64-7d70b9ca3759
md"""
Reimplementation of <https://arogozhnikov.github.io/2023/12/28/fastest-autograd.html> in the Julia [autodiff ecosystem](https://juliadiff.org/).
"""

# ╔═╡ 35821784-4e86-4b63-803f-f7786d360114
md"""
## Setup
"""

# ╔═╡ 405c8752-b921-4651-9692-4205a21386b1
TableOfContents()

# ╔═╡ 3d9d5d86-c002-4154-83eb-b64877f62c4c
Threads.nthreads()

# ╔═╡ 56b4fd43-a3f2-47c7-85ab-4d4114f83d38
md"""
## Function to differentiate
"""

# ╔═╡ bf35dd1f-0dc9-4261-9acc-e104088c3d6e
function run_graph(initial_variables::AbstractVector, n_op::Integer=10^5)
	nodes = copy(initial_variables)
	for op in 0:n_op-1
		case = op % 4
		if case == 0
			push!(nodes, softplus(nodes[end-9]))
		elseif case == 1
			push!(nodes, sum(@view(nodes[end-29:5:end-10])))
		elseif case == 2
			push!(nodes, nodes[end-19] * nodes[end-9])
		else
			softmaxes = softmax(@view(nodes[end-3:end]))
			append!(nodes, softmaxes)
		end
	end
	return nodes[end]
end

# ╔═╡ d032884d-2467-47e5-911d-26749f8ea135
function run_graph_fast(initial_variables::AbstractVector{T}, n_op::Integer=10^5) where T
	n_var = length(initial_variables)
	nodes = similar(initial_variables, n_var + 2n_op)
	nodes[1:n_var] .= initial_variables
	k = n_var
	for op in 0:n_op-1
		case = op % 4
		if case == 0
			val = softplus(nodes[k-9])
			nodes[k+1] = val
			k += 1
		elseif case == 1
			inds = (k-29):5:(k-10)
...
# ╔═╡ 3f837519-ba73-45ce-9583-1be3ea8d176a
@testset begin
	init_vars = float.(1:100)
	@testset for n_op in 0:100
		@test run_graph(init_vars, n_op) ≈ run_graph_fast(init_vars, n_op)
	end
end

# ╔═╡ 378d6d38-6d67-40da-b7ba-2e0e5986ef32
@be float.(1:100) run_graph

# ╔═╡ f2c7662c-2f99-4e25-81a2-15b407f6f738
@be float.(1:100) run_graph_fast

# ╔═╡ c116d26c-eecd-4981-a531-63281e64fe76
md"""
Note: due to numerical underflow, the gradient is quickly zero everywhere, but it should not affect computation time.
"""

# ╔═╡ 7c10da4d-9680-4162-b724-80e1641087e3
unique(gradient(run_graph_fast, AutoForwardDiff(), float.(1:100), Constant(10)))

# ╔═╡ 38f94363-fccb-4046-8c43-17dde1aa873c
unique(gradient(run_graph_fast, AutoForwardDiff(), float.(1:100), Constant(100)))

# ╔═╡ 923f9c59-914d-43e1-8d64-f97c71269b2b
md"""
## Benchmark
"""

# ╔═╡ dd84cb79-21b4-4cf4-a4cb-c79079f0486a
scen = Scenario{:gradient, :out}(
	run_graph_fast, Float32.(1:100)
)

# ╔═╡ 32a3201a-c1ec-43ab-bc6f-0a5a9d91322c
backends = [
...
# ╔═╡ 34f91a56-3c8b-4a5e-b262-b966bb7b0a6b
let
	init_vars = float.(1:30)
	gradient(run_graph_fast, AutoReverseDiff(), init_vars, Constant(100))
end

# ╔═╡ df62cbb6-9a1c-4655-a788-21c9a3bdc397
...
processed_data = @chain data begin
	@rsubset(:operator == Symbol("gradient"))
	@orderby(:time)
end

# ╔═╡ d30d8f6e-d066-454d-9213-ea9ed906fc25
nameof.(typeof.(processed_data[!, :backend]))

# ╔═╡ b76287fd-b653-4eb9-969b-ac990faac368
let
	n = size(processed_data, 1)
	backends = processed_data[!, :backend]
	names = string.(nameof.(typeof.(backends)))
	modes = string.(nameof.(typeof.(DifferentiationInterface.mode.(backends))))
	labels = [n == "AutoEnzyme" ? "$(n[5:end]) ($(m[begin:end-4]))" : n[5:end] for (n, m) in zip(names, modes)]
	fig = Figure(size=(600, 500))
	ax = Axis(
		fig[1, 1], xlabel="autodiff package", ylabel="time (s)", yscale=log10,
		xticks=(1:n, labels), xticklabelrotation=1
	)
	barplot!(ax, float.(1:n), processed_data[!, :time])
	fig
end
...
DifferentiationInterfaceTest = "~0.9.0"
Enzyme = "~0.13.16"
...
manifest_format = "2.0"
project_hash = "311dca283797bf2ffd6013c2e96a27e1db2cd87e"
...
deps = ["ADTypes", "AllocCheck", "Chairmarks", "DataFrames", "DifferentiationInterface", "DocStringExtensions", "JET", "LinearAlgebra", "ProgressMeter", "Random", "SparseArrays", "Test"]
...
git-tree-sha1 = "f56cc86a351767fcb3bb2890b6e1a14d0ef323ab"
...
deps = ["DataStructures", "DiffRules", "LinearAlgebra", "NaNMath", "RuntimeGeneratedFunctions", "SparseArrays", "SpecialFunctions", "StaticArrays", "UUIDs"]
git-tree-sha1 = "41fed8911392e9623a3861ee91f208ad2b474b3e"
...
git-tree-sha1 = "ed4b983e826c0d205ceede5b8eb4c14e47c69171"
...
git-tree-sha1 = "5cd736337b9dc0de9bfe894f067f300274fceab1"
